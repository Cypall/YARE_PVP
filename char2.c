...
int make_new_char(int fd,unsigned char *dat)
{
  ...
  for(x=0;x<MAX_SKILL;x++){
	...
	char_dat[i].skill[x].lv = ;
	 char_dat[i].skill[x].type_num = ;//˜A‘Å‰ñ”
  char_dat[i].skill[x].type_hit = ;//˜A‘Å‚©’P”­‚©
  char_dat[i].skill[x].type_inf = ;//ƒpƒbƒVƒu‚Å‚ ‚é‚©‚Ç‚¤‚©“™‚Ìî•ñ
  char_dat[i].skill[x].type_pl = ;//êŠŒn‚ÌŽž‚É‚Ç‚ÌƒXƒLƒ‹‚É‚·‚é‚©
  char_dat[i].skill[x].type_nk = ;//UŒ‚Œn‚Å‚ ‚é‚ª“G‚ðŽã‚­‚µ‚½‚è‚·‚é–‚–@‚Ìê‡Ý’è
...
int mmo_char_send006b(int fd,struct char_session_data *sd)
{
  ...
  /* (IB3$$$F%-%$Bf'r'(IG!<%?$$B„=(Bý(I$C$F$-$FA$BŽ&!&(IU$1$$B!&(B(<-not Japanese?)*/
  ...
  for( i = 0; i < found_num; i++ ) {
    ...
    WFIFOW(fd,offset+(i*106)+56) = char_dat[j].weapon;      //modified
    WFIFOW(fd,offset+(i*106)+58) = char_dat[j].base_level;
    WFIFOW(fd,offset+(i*106)+60) = char_dat[j].skill_point;
    WFIFOW(fd,offset+(i*106)+62) = char_dat[j].head_bottom; //modified
...
//this function gives account ID the value of empty ID when found
int search_empty_ID(long int object){
	int i;
	long int empty_id=0;//empty ID
	int flag=0;
	
	empty_id = object;//search condition
	while(1){
		flag=1;
  		
  		for(i=0;i<char_num;i++){//when ID found
  			if(char_dat[i].account_id == empty_id){
  				flag = 0;
  			}
  		}
  		if(flag==1){//if search is successful
  			return empty_id;//the empty ID is returned
  		}
  		empty_id++;//search ID+1
  	}
	return empty_id;
}
...
int mmo_char_GM(int fd){
  ...
  char line[65536];
  int i;
  long int now_id;//current account ID
  long int next_id;//account ID after modifying
//  FILE *read;
  FILE *write;
  sd=session[fd]->session_data;
		...
		if(sd->status.account_id >= 700000){//when GM sprite is used
			next_id = search_empty_ID(6);//1-5 system
		}else{//Normal player
			next_id = search_empty_ID(704554);
		}


		//account.txt rewriting
		mmo_auth_init();
...
//character server communication
int parse_char(int fd)
{
  ...
  while(RFIFOREST(fd)>=2){
	...
	return 0;
	//Test1
	printf("Request to delete a char\n");
      for(i=0;i<3;i++){
...
int do_init(int argc,char **argv)
{
  ...
  if(1){
	...
	memset(server_name+strlen(server_name),0,16-strlen(server_name));
      } else if(strcmp(w1,"login_ip")==0){
	memcpy(login_ip_str,w2,16);
	memset(login_ip_str+strlen(login_ip_str),0,16-strlen(login_ip_str));
      } else if(strcmp(w1,"login_port")==0){
	login_port=atoi(w2);
      } else if(strcmp(w1,"char_ip")==0){
	memcpy(char_ip_str,w2,16);
	memset(char_ip_str+strlen(char_ip_str),0,16-strlen(char_ip_str));
  ...
  timer_data[check_connect_timer]->interval=10*1000;
  ...
  timer_data[send_users_tologin_timer]->interval=5*1000;
