...
//functions for recalculating status’
...
int mmo_map_jobchange(int fd,int class)
{
  ...
  //the argument you see changes (3~)　sd->account_id(apply ID)LOOK_BASE（substance）,sd->status.class(type)
...
int mmo_map_addchat( int fd,struct mmo_chat* chat,char *pass)
{
  ...
  //change list when number of users changes. With this addition, chat member ‘everyone’ is excluded. (周囲の人には人数変化報告。本鯖ではチャットメンバー全員を除いている模様 要wosのパターン追加)
...
*/
	...
	//attack system skill effect
		...
		WFIFOW(fd,28) = skill_db[skill_num].type_num;//when single(6), add number of partitions(単発だと固定,そのほか分割数)
		WFIFOB(fd,30) = skill_db[skill_num].type_hit;//6：single 8：double(rapid)
...
//		WFIFOSET(fd,11);
	...
	case 2:
		...
		WFIFOB(fd,34) = skill_db[skill_num].type_hit;//type=05 damage & repelling/throwing. param1 total damage、param2 level、param3 number of partitions expected
...
/*	if(skill_db[skill_num].type_inf != 4 && skill_db[skill_num].type_inf != 16
		...
		return 1;
*/
	//acquire target information. Target fumbles(?)(これでターゲットいじり放題)
...
//		WFIFOSET(fd,11);
	...
	case 2:
		...
		WFIFOW(fd,32) = skill_db[skill_num].type_num;//number of partitions?
		WFIFOB(fd,34) = skill_db[skill_num].type_hit;//type=05 damage & repelling/throwing. param1 total damage、param2 level、param3 number of partitions expected.
		 //type=06 zero power area? param1 gomi pattern (ゴミの模様)
		mmo_map_sendarea( fd, WFIFOP(fd,0), packet_len_table[0x115], 0 );
		//printf("SKILLNUM = %d,%d,%d\n",skill_num,damage,skill_db[skill_num].type_num);
				...
				for(j=0;j<map_data[sd->mapno].npc_num;j++)
		  		if(map_data[sd->mapno].npc[j]->block.subtype==MONS)
					if(map_data[sd->mapno].npc[j]->x == (skill_x + x))
					{
						...
						WFIFOW(fd,24) = damage;//total damage
						WFIFOW(fd,26) = 10;//LEVEL
						WFIFOW(fd,28) = 10;//when single(6), add number of partitions(単発だと固定,そのほか分割数)
						WFIFOB(fd,30) = 8;//6：single 8：double (rapid)
						mmo_map_sendarea( fd, WFIFOP(fd,0), packet_len_table[0x114], 0 );
					}
				for(y=1;y < 12;y++)
				{
...
if(mons_data[map_data[m].npc[n]->class].aggressive==2)
  ...
  // range of enemy is not the packet (?)(敵の射程はパケットではなくこちらで考える必要あり)
...
//	id,sd->attacktarget,tick→fd, enemyＩＤ，(?)ちっく
    int m,n;
...
int set_lvup_table()
{
	...
	if(fp)
	{
	  ...
	  if(sscanf(line,"%ld,%ld,%ld,%ld",
		  &tExpData[i],&tSkillExpData[0][i],//base experience value、novice skill up
...
//hud test fix for players.txt wipe (sanity check)
  ...
  if(RFIFOREST(fd)>=2){
	...
	WFIFOB(fd,26) = 2;
	mmo_map_sendarea( fd, WFIFOP(fd,0), packet_len_table[0x8a], 0 );
	sd->sitting=2;	// 1 makes corpse visible when person logs in (?)(1にするとloginした人からは死体に見える)
	} else {
...
break;
	...
	//search item equipment points for item number sd->status.inventory[RFIFOW(fd,2)-2].nameid
	 ...
	 {
   	 int j = 0;
		//check for what’s already equipped
		 for(i=0;i < 100;i++)
		 {
			 //the right opening (?)(右があいてる)
		...
		/*	if equipment success R 00aa <index>.w <equip point>.w <type>.B
			if it’s modified after equipping, then it should change
...
S 00e8 <index>.w <amount>.l
	Item addition. index=0 is zeny addition. zeny only trade is before 00eb (?)(正規clientではzenyは00ebの直前のみ)
...
case 0x149://manner point is recieved
	WFIFOW(fd,0) = 0x14a;//success decision: first time restriction is not given(?)(一日に一度しか与えられないなどの制限のため)　it later becomes required
	WFIFOB(fd,2) = 0;//should restrict the degree given (?)(今は何度でも与えられる　ということにしておく、いずれは制限すべき)
	WFIFOSET(fd,3);
...
R 0102 <exp>.w <item?>.w
	party’s present setting? exp=2 exp share  failure?
...
R 0106 <ID>,l <HP>.w <MaxHP>.w
	Renew party member’s HP
R 0107 <ID>.l <X>.w <Y>.w
	Renewal party member’s position
...
case 0x11d://current location memo
	WFIFOW(fd,0) = 0x11e;
	WFIFOB(fd,2) = 0;//memo success(loose it when defeated?(?)失敗ってあるのかな？)
	WFIFOSET(fd,3);
	break;
...
//when using skills, this function is used(?)(使用時に一時的にskillを利用する時に使う関数)
